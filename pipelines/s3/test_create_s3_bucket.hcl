pipeline "test_create_s3_bucket" {
  ...
  */
  ...
  output "create_s3_bucket_check" {
    description = "Checks for pipeline.create_s3_bucket."
    value       = "step.pipeline.create_s3_bucket ${step.pipeline.create_s3_bucket.stderr == "" ? "succeeded" : "failed: ${step.pipeline.create_s3_bucket.stderr}"}"
  }

  output "list_s3_buckets_check" {
    description = "Checks for pipeline.list_s3_buckets."
    value       = "step.pipeline.list_s3_buckets ${step.pipeline.list_s3_buckets.stderr == "" && length([for bucket in step.pipeline.list_s3_buckets.stdout.Buckets : bucket if bucket.Name == param.name]) > 0  ? "succeeded" : "failed: ${step.pipeline.list_s3_buckets.stderr}"}"
  }

  output "delete_s3_bucket_check" {
    description = "Checks for pipeline.delete_s3_bucket."
    value       = "step.pipeline.delete_s3_bucket ${step.pipeline.delete_s3_bucket.stderr == "" ? "succeeded" : "failed: ${step.pipeline.create_s3_bucket.stderr}"}"
  }
